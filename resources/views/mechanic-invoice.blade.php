@php
    $servicesTotal = $record->services->sum('price');        // from repeater
    $partsTotal    = $record->partUsages
                        ->sum(fn ($u) => $u->quantity * $u->unit_price);
    $totalCost     = $servicesTotal + $partsTotal;
    $mechAmount    = round($totalCost * $mechanicPct / 100, 2);
@endphp
...
<div style="font-family: Arial, sans-serif; direction: rtl; color:#000; font-size:14px; padding:20px;">
    ...
    <div style="display:flex; align-items:center; border-bottom:2px solid #3e2f92; padding-bottom:10px; margin-bottom:20px;">
        <img src="{{ asset('storage/logo.jpg') }}" alt="logo" style="width:60px; height:auto; margin-left:20px;">
        <div>
    ...
    <table style="width:100%; border-collapse:collapse; margin-top:20px;">
        ...
        <tr>
            <td style="padding:5px;"><strong>إجمالي التكلفة:</strong></td>
            <td style="padding:5px;">{{ number_format($totalCost,2) }} د.ل</td>
        </tr>
        <tr>
            <td style="padding:5px;"><strong>نسبة الفني:</strong></td>
